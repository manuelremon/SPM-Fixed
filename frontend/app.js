// En desarrollo cuando el frontend se sirve desde el puerto 8080 (python -m http.server)
// queremos apuntar las llamadas API al backend en el puerto 5000.
const API = (function () {
  // si estamos sirviendo desde el puerto 8080 (dev), redirigir al backend en 5000
  if (location.port === "8080") {
    return "http://127.0.0.1:5000/api";
  }
  // cuando se abre como archivo local, también usar backend local por conveniencia
  if (location.protocol === "file:") {
    return "http://127.0.0.1:5000/api";
  }
  // comportamiento por defecto: usar el origin actual + /api
...
function renderAdminNav() {
  ...
  const adminLinks = [
    ...
    { href: "admin-usuarios.html", label: "Editar usuarios" },
    { href: "admin-solicitudes.html", label: "Solicitudes (todas)" },
    { href: "admin-materiales.html", label: "Catálogo de materiales" },
    { href: "admin-centros.html", label: "Administrar centros" },
    { href: "admin-almacenes.html", label: "Almacenes virtuales" },
    { href: "admin-reportes.html", label: "Reportes y auditoría" },
    { href: "admin-configuracion.html", label: "Configuración general" },
  ];

  const linksHtml = adminLinks
    .map((item) => {
      const activeClass = current === item.href ? "active" : "";
      return `<a href="${item.href}" class="${activeClass}">${item.label}</a>`;
    })
    .join("");

  adminNav.innerHTML = `<span class="nav-heading">Administración</span>${linksHtml}`;
